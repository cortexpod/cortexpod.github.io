...
```jsx
import { useRouter } from "next/router"
import { useConfig } from "cortexpod-theme-docs"
...
<div className="mt-8 text-center text-sm">[Live example on StackBlitz ↗](https://stackblitz.com/edit/cortexpod-2-docs-yrlccm?file=theme.config.jsx)</div>
...
A banner can be dismissed. By default `banner.key` will be `"cortexpod-banner"` and
it’s used by
...
<div className="mt-8 text-center text-sm">[Live example on StackBlitz ↗](https://stackblitz.com/edit/cortexpod-2-docs-8xbfk3?file=pages%2F_meta.json,theme.config.jsx)</div>
